...
from transformers import AutoTokenizer, HfArgumentParser, pipeline, AutoModelForSequenceClassification, AutoModelForCausalLM
...
class ScriptArguments:
    ...
    model_name: Optional[str] = field(default="/data/xyq/trl/non-toxical/llama-2-7b-hh-fim", metadata={"help": "the model name"})
...
# its own dataset.
def build_dataset(config, dataset_name="Anthropic/hh-rlhf", input_min_text_length=2, input_max_text_length=32):
    """
    ...
    ds = ds.rename_columns({"chosen": "text"})
    ds = ds.filter(lambda x: len(x["text"]) > 200, batched=False)

    input_size = LengthSampler(input_min_text_length, input_max_text_length)

    def tokenize(sample):
        # the text : '\nHuman: How do I take a book out of the library without having to return it?\n\nAssistant: I’m not sure what you'
        # extract what the Human says (How do I take a book out of the library without having to return it?)
        # Find the index of "Human:" and "Assistant:"
        human_index = sample["text"].find("\n\nHuman:")
        assistant_index = sample["text"].find("\n\nAssistant:")
        # print("sample", sample["text"])
        # Extract the human's question
        if human_index != -1 and assistant_index != -1:
            human_question = "\n\nHuman:" + sample["text"][human_index + len("\n\nHuman:"):assistant_index].strip()
            # print(human_question)
        # else:
        #     print("No human question found.")
        # print("human_question", human_question)
        # input("hold")
        sample["input_ids"] = tokenizer.encode(human_question)[: input_size()]
...
rank_model, reward_tokenizer = AutoModelForSequenceClassification.from_pretrained(reward_name).to(device), AutoTokenizer.from_pretrained(reward_name)
...
for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
    ...
    # print("response device: ", response_tensors[0].device)
    # print("query device: ", query_tensors[0].device)
    # print("rank model device: ", rank_model.device)
    # input()
    # print("query_tensors", query_tensors)
    # print("response_tensors", response_tensors)
    # inputs = torch.cat([query_tensors, response_tensors])
    # inputs = [torch.cat((q, r), dim=0) for q, r in zip(query_tensors, response_tensors)]
    inputs = response_tensors
    # inputs_text = tokenizer.batch_decode(inputs)
    # print("inputs_text", inputs_text)
    # inputs_text = tokenizer.batch_decode(query_tensors)
    # response_text = tokenizer.batch_decode(response_tensors)
    # print("query", inputs_text)
    # print("response_text", response_text)

    # test = tokenizer(inputs_text[0], return_tensors='pt')
    # print("test",test)
    # print("inputs[0]", inputs)

    # print("inputs", len(query_tensors[0]))
    # print("inputs", len(response_tensors[0]))
    # print("len of inputs", len(inputs))
    # print("inputs", len(inputs[0]))
    
    rewards = [rank_model(**{"input_ids":input.unsqueeze(dim=0)}).logits[0].cpu().detach() for input in inputs]
    # print("rewards device:", rewards[0].device)
    # print("score", score)
